...
## 분할 영역 지정  
## block과 inline의 차이는?  
HTML요쇼의 타입 : 모든 요소는 해당 요소가 웹 브라우저에 어떻게 보이는가를 결정짓는 display속성을 사짐, 대부분의 요소는 display속성 값으로 block/inline을 가진다.    

1. display 속성 값이 **block**이라면   
새로운 line에서 시작, 해당 라인의 모든 너비를 차지!   
대표적요소 : ```<p>, <div>, <h>, <ul>, <ol>, <form>```   
- ```<div>``` : 다른 html요소들을 하나로 묶는 대표적인 block요소! 여러 요소들의 스타일을 한 번에 적용하기 위해 사용     

2. display 속성 값이 **inline**이라면   
새로운 라인이 아닌 해당 라인에서 시작(in line)   
요소의 너비도 해당 라인 전체가 아닌 '해당 html요소의 내용 만큼'만 차지!  
대표적요소 : ```<span>, <a>, <img>```  
- ```<span>```요소 : **'텍스트의 특정 부분'**을 묶는데 자주 사용하는 인라인 요소  
