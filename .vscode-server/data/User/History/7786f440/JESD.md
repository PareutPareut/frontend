...
>**서비스 계층   : 요청에 필요한 요구사항을 구현**
하나의 페이지가 구현해야하는 기능 중 저수준의 데이터 처리로직(SQL수행)과 웹 고유의 기능(요청에 대한 응답, UI구현)을 분리하고, 이를 연결해주는 역할 수행  
데이터 베이스를 처리하는 코드를 모듈화 시키는 것이 서비스 계층의 취지
