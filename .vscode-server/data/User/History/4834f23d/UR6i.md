...
미들웨어가 존재하지 않더라도 express가 자체적으로 404 에러를 처리해주기는 하지만, 웬만하면 연결해라!  
```JavaScript
app.use((req,res,next)=>{
  res.status(404).send('Not Found');
});
```
이 미들웨어를 제거 후 localhost:3000/abc에 접속->404상태 코드와 Cannot Get / abc라는 메세지가 응답됨.  
...
7. nodeJS 교과서 -6.3 router객체로 라우팅 분리하기  
---