...
**3. absolute**, 절대 위치  
position : absolute;
- static이 아닌 부모를 만나면(position type이 지정이 된) 그 **부모**를(grand) **기준**으로 left/top의 값을 정하게 된다.(위치 지정)
- 부모와의 관계가 끊기기 때문에  
...
4.fixed  
스크롤을 내려도 그 자리에 계속 있도록


**1. static VS relative**  
```html
#me{
            position: relative; 이 속성이 없으면 이동하지 않음
            left:100px; 오른쪽으로 100px이동
            top:100px; 위에서 아래로 100px만큼 내려오도록
        }
```
- left가 right보다 우선, top이 bottom보다 우선된다.  
- 각각의 element들은 static이라는 기본값을 가진다. 그래서 이동시키고 싶다면 position type을 relative로 지정해야한다! (**position: relative;**를 지정해주어야 한다.)
